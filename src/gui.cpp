...
namespace rise {
	...
	void processImguizmo(entt::registry& r) {
		auto instance = r.ctx<Instance*>();
		auto& state = r.ctx<GuiResources>();

		auto& model = r.get<impl::ModelRes>(state.selectedEntity);

		auto pData = mapUniformBuffer<impl::ModelData>(instance->renderer.get(), model.uniformBuffer);
		auto pGlobal = mapUniformBuffer<impl::GlobalShaderData>(instance->renderer.get(),
			instance->globalShaderData);

		ImGuiIO& io = ImGui::GetIO();
		ImGuizmo::SetRect(0, 0, io.DisplaySize.x, io.DisplaySize.y);

		ImGuizmo::Manipulate(&pGlobal->view[0][0], &pGlobal->projection[0][0],
			state.guizmoOperation, state.guizmoMode, &pData->transform[0][0], nullptr, nullptr);

		glm::vec3 matrixTranslation, matrixRotation, matrixScale;
		ImGuizmo::DecomposeMatrixToComponents(&pData->transform[0][0],
			&matrixTranslation.x, &matrixRotation.x, &matrixScale.x);

		if (r.try_get<Position>(state.selectedEntity)) {
			r.replace<Position>(state.selectedEntity, matrixTranslation);
		}
		if (r.try_get<Rotation>(state.selectedEntity)) {
			r.replace<Rotation>(state.selectedEntity, matrixRotation);
		}
		if (r.try_get<Scale>(state.selectedEntity)) {
			r.replace<Scale>(state.selectedEntity, matrixScale);
		}

		instance->renderer->UnmapBuffer(*model.uniformBuffer);
		instance->renderer->UnmapBuffer(model.uniformBuffer);
	}
	...
	void recordImgui(entt::registry& r) {
		...
		if (ImGui::RadioButton("Translate", state.guizmoOperation == ImGuizmo::TRANSLATE)) {
			state.guizmoOperation = ImGuizmo::TRANSLATE;
		}
		ImGui::SameLine();
		if (ImGui::RadioButton("Rotate", state.guizmoOperation == ImGuizmo::ROTATE)) {

			state.guizmoOperation = ImGuizmo::ROTATE;
		}
		ImGui::SameLine();
		if (ImGui::RadioButton("Scale", state.guizmoOperation == ImGuizmo::SCALE)) {
			state.guizmoOperation = ImGuizmo::SCALE;
		}
		...
		writeChapter<Drawable>(r, "Drawable objects: ", [&state](entt::registry& r, entt::entity e) {
